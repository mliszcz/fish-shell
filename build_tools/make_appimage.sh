...
env CFLAGS=-std=gnu99 cmake3 -S "$ROOT_DIR" -B "$APP_BUILD_DIR" \
...
env CC=/usr/bin/c99 make -C "$APP_BUILD_DIR" -j$(nproc)
env CC=/usr/bin/c99 make -C "$APP_BUILD_DIR" install DESTDIR="$APP_DIR"
